import Vector2 = Phaser.Math.Vector2

const Constants = {
    BALL_RADIUS: 20,
    BALL_START_POSITION: new Vector2(0, 300),
    BALL_X_VELOCITY: 4,
    PLATFORM_THICKNESS: 15,
    PLATFORM_TINT: 0x777684,
    PLATFORM_HIT_MIDDLE_TINT: 0x55ff55,
    PLATFORM_HIT_EDGE_TINT: 0xff7755,
    GRADIENT_COLUMN_DEFAULT_TINT: 0xced6e3,
    GRADIENT_COLUMN_RANDOM_TINTS: [0xD3F8E2, 0xE4C1F9, 0xF694C1, 0xEDE7B1, 0xA9DEF9],
    PLATFORM_POOL_SIZE: 10,
    PLATFORM_MIDDLE_PERCENTAGE: 0.60,
    HIGH_SCORE_STORAGE_KEY: 'high-score',
    GEMS_COLLECTED_STORAGE_KEY: 'gems-collected',
    SCREENSHAKE_DURATION: 100,
    SCREENSHAKE_STRENGTH: 0.0075,
    GEM_SPAWN_CHANCE: 0.25,
}

export default Constants